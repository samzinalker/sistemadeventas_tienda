...
$nro_venta = isset($_GET['nro_venta']) ? $_GET['nro_venta'] : 0;

if (!$id_venta_get && !$nro_venta) {
...
if ($id_venta_get) {
    ...
    $sql_ventas = "SELECT ve.*, cli.nombre_cliente as nombre_cliente
                   FROM tb_ventas as ve 
                   INNER JOIN tb_clientes as cli ON cli.id_cliente = ve.id_cliente 
                   WHERE ve.nro_venta = :nro_venta AND ve.id_usuario = :id_usuario";
    $query_ventas = $pdo->prepare($sql_ventas);
...
// Incluimos la consulta del porcentaje_iva si existe
